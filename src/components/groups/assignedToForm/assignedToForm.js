...
class AssignedToForm extends Component {
    ...
    handleSubmit = (event) => {
        ...
        axios.put(GROUP_PUT_URL, data, {withCredentials: true, crossDomain: true}).then(response => {
            // eslint-disable-next-line no-console
    ...
    render() {
        return (
            <form className="forms_list">
                {    // eslint-disable-next-line react/prop-types
                    this.state.forms && this.state.forms.map((form) => {
                        if (this.state.checked_forms.indexOf(form.form_id) >= 0) {
                            return (
                                <div className='forms' key={form.form_id}>
                                    <input type="checkbox"
                                           checked={true}
                                           className="groups"
                                           id={form.form_id}
                                    ...
                                    <label className='forms_title' htmlFor={form.form_id}>{form.title}</label>
                                </div>);
                        } else {
                            return (
                                <div className='forms' key={form.form_id}>
                ...
                <input type="submit" onClick={this.handleSubmit} className='btn btn-outline-dark btn_submit' value="Submit"/>
            </form>);
    }
}
